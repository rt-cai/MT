...
uthread_mutex_t mx;
uthread_cond_t proCanGo, conCanGo;
...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    ...
    while(items >= MAX_ITEMS)
      uthread_cond_wait(proCanGo);
    items += 1;
    uthread_cond_signal(conCanGo);
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_mutex_lock(mx);
    while(items <= 0)
      uthread_cond_wait(conCanGo);
    items -= 1;
    uthread_cond_signal(proCanGo);
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_mutex_unlock(mx);
  }
...
int main (int argc, char** argv) {
  ...
  proCanGo = uthread_cond_create (mx);
  conCanGo = uthread_cond_create (mx);
